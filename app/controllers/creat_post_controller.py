...
def create_post_controller(post: PostSchema, files):
    try:
        ...
        my_collection: Collection[dict] = db.get_collection(collection_name)
        ...
        # Ensure comments is always an empty list when creating a post
        post_document = {
            "likes": post.likes,
            "shares": post.shares,
            "postTypes": post_type,
            "comments": [],  # Explicitly setting comments as an empty list
            "username": post.username,
            "files": file_names,
        }
